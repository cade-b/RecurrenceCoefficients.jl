...
function pre_comp_no_circ(h, bands, nmat, typemat)
    # get all necessary ApproxFun coefficients at once
    int_vals = cheby_int(bands)
    gap_vals = cheby_gap(bands)
...
function get_n_coeffs_no_circ(bands,n::Int64,typemat=nothing,h=nothing;nmat=nothing)
    if typemat == nothing
    ...
    pre_comp_no_circ(h, bands, nmat, typemat)
...
function get_n_coeffs_no_circ(bands,nstart::Int64,nend::Int64,typemat=nothing,h=nothing;nmat=nothing)
    if typemat == nothing
        typemat = special_type(bands)
    end
    ...
    pre_comp_no_circ(h, bands, nmat, typemat)
    Y₁ = main_comp_no_circ(bands,nmat,nstart)
    ...
    for j = nstart:nend
        ...
        avec[j-nstart+1] = real(a)
        bvec[j-nstart+1] = real(b)
        Y₁ = Y₁₊
    end
    (avec,bvec)
end
